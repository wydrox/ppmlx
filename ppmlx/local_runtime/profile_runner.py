...
from .profile_publication import classify_report, validate_report
...
class AppleEvaluationEnvironment:
    """Apple Silicon environment metadata required by the publication contract."""
...
class GenerationSettings:
    """Deterministic settings shared by all three evaluation runs."""
...
class TextEngineProfileGenerator:
    """Apple Silicon MLX generator used by the manual evaluation command."""
    ...
    *,
    ...
    for case in case_set.cases:
        try:
            ...
            cases.append(_generation_failure(case))
            continue
        cases.append(
            evaluate_generated_output(
                output,
    ...
    return RunEvaluation(run_index=run_index, seed=seed, cases=tuple(cases))
...
def _finalize_report(
    ...
    *,
    ...
    environment_data = dict(report["environment"])  # type: ignore[arg-type]
    environment_data["architecture"] = environment.architecture
    report["environment"] = environment_data

    case_set_data = dict(report["case_set"])  # type: ignore[arg-type]
    case_set_data["sha256"] = case_digest
    report["case_set"] = case_set_data

    runs = list(report["runs"])  # type: ignore[arg-type]
    aggregate = dict(report["aggregate"])  # type: ignore[arg-type]
    aggregate["maximum_run_repaired_valid_call_rate"] = max(
        float(run["repaired_valid_call_rate"])  # type: ignore[index]
        for run in runs
    )
...
def run_profile_evaluation(
    *,
    ...
    """Run three fixed evaluations and return one validated content-free report."""

    if type(model_path) is not str or not model_path:
        raise ProfileRunnerError("model_path_required")
    selected_commit = ppmlx_commit or current_git_commit(repository_root)
    runs = tuple(
        _run_once(
            run_index=index,
            seed=seed,
    ...
    return _finalize_report(
        ...
        case_digest=case_set_sha256(case_set_path),
    )
